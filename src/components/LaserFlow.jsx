...
export const LaserFlow = ({
    ...
    useEffect(() => {
        ...
        const adjustDpr = now => {
            ...
            if (!samples.length) { lastFpsCheckRef.current = now; return; }
            const avg = samples.reduce((a, b) => a + b, 0) / samples.length;
            let next = currentDprRef.current;
            const base = baseDprRef.current;
            if (avg < 50) next = clamp(next * 0.85, 0.6, base);
            else if (avg > 58 && next < base) next = clamp(next * 1.1, 0.6, base);
            if (Math.abs(next - currentDprRef.current) > 0.01 && now - lastDprChange > 2000) {
                currentDprRef.current = next; lastDprChange = now; setSizeNow();
            }
