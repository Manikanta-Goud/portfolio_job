...
#define PI_OVER_3 1.04719755
#define PI_OVER_6 0.52359878

#define M1 1664525u
#define M2 1013904223u
#define M3 1000003u
...
uint coord3(ivec3 p) {
  return uint(p.x) * M1 ^ uint(p.y) * M2 ^ uint(p.z) * M3;
}

vec3 hash3f(uint n) {
  uvec3 h = hash(n) * uvec3(1u, 511u, 262143u);
  return vec3(h) * F0;
}
...
void main() {
  ...
  for (int i = 0; i < 128; i++) {
    ...
    float cellHash  = hash3f(cellCoord).x;

    if (cellHash < uDensity) {
      vec3 h = hash3f(cellCoord);
      vec3 sinArg1  = vec3(fpos.yzx) * 0.073;
      vec3 sinArg2  = vec3(fpos.zxy) * 0.27;
      vec3 flakePos = 0.5 - 0.5 * cos(4.0 * sin(sinArg1) + 4.0 * sin(sinArg2) + 2.0 * h + timeAnim);
      flakePos = flakePos * 0.8 + 0.1 + vec3(fpos);
      ...
      if (toIntersection > 0.0) {
        ...
        if      (uVariant < 0.5) { dist = max(testUV.x, testUV.y); }
          ...
          float invFS = 1.0 / flakeSize;
          dist = snowflakeDist(vec2(testX, testY) * invFS) * flakeSize;
        }

        if (dist < flakeSize) {
          float ratio     = uFlakeSize / flakeSize;
          float intensity = exp2(-(t + toIntersection) * invDepthFade) *
                            min(1.0, ratio * ratio) * uBrightness;
...
export default function PixelSnow({
    ...
    return (
        <div ref={containerRef} className={`pixel-snow-container ${className}`} style={style} />
    );
}
